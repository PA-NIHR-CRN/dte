...
export default class HealthConditionsPage {
  ...
  constructor(page: Page) {
    ...
    this.healthConditionHintText = page.locator(
      `p:text("This includes any physical and mental health conditions or illnesses.")`
    );
    ...
    this.summaryText2 = page.locator(
      `span:text("If we find that people with disabilities are under represented in signing up to be contacted about research we will look at how to improve this.")`
    );
    ...
    this.continueButton = page.getByRole("button", { name: "Save" });
  ...
  async assertFormVisible() {
    await assertComponentsVisible([
      ...
      this.radioButtonNo,
      this.labelNo,
      this.radioButtonPreferNotToSay,
      this.labelPreferNotToSay,
    ]);
  }
  ...
  async assertTextVisible() {
    await assertComponentsVisible([
      ...
      this.healthConditionsPageHeading,
      this.healthConditionHintText,
