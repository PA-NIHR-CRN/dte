...
import { render, fireEvent, screen, waitFor, userEvent } from "../../../Helper/test-utils";
...
describe("MobileNumberForm", () => {
  it("renders the form", () => {
    ...
    render(<MobileNumberForm initialStateData={data} onDataChange={mockOnDataChange} />);
  });
  ...
  it("displays an errror when an invalid number is entered in the mobile field", async () => {
    ...
    expect(
      await screen.findByText("Enter a valid mobile number, like 07700 900 982 or +44 7700 900 982")
    ).toBeInTheDocument();
  });
  ...
  it("displays an errror when an invalid number is entered in the landline field", async () => {
    ...
    fireEvent.click(screen.getByText("Continue"));
    expect(
      await screen.findByText("Enter a valid landline number, like 01632 960 001 or +44 1632 960 001")
    ).toBeInTheDocument();
  });
...
describe("Accessibility test", () => {
  it("should not fail any accessibility tests", async () => {
    const mockOnDataChange = jest.fn();
    const { container } = render(<MobileNumberForm initialStateData={data} onDataChange={mockOnDataChange} />);
