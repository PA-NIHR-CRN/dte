...
import { render, screen, userEvent, waitFor, waitForElementToBeRemoved } from "../../../Helper/test-utils";
...
beforeEach(() => {
  ...
  server = createServer({
    routes() {
      this.delete(`${process.env.REACT_APP_BASE_API}/users/deleteparticipantaccount`, () => {
        return {
          content: null,
          isSuccess: true,
          errors: [],
...
describe("Close Account display tests", () => {
  ...
  it("must display the data retrieved from the server correctly", async () => {
    ...
    expect(await screen.findByText("To take part in the future you can register again.")).toBeInTheDocument();

    const links = await screen.findAllByRole("link");
    expect(links).toHaveLength(2);
    expect(links[0]).toHaveAttribute("href", "/");
    expect(links[0]).toHaveTextContent("Back");
    expect(links[1]).toHaveAttribute("href", "https://bepartofresearch.nihr.ac.uk/site-policies/privacy-policy/");
...
describe("Close Account functional tests", () => {
  it("must display warning dialogue on Close your account", async () => {
    ...
    expect(await screen.findByText("Confirm if you want to close your account")).toBeInTheDocument();
  ...
  it("must handle cancel close account correctly", async () => {
    ...
    expect(screen.queryByText("Confirm if you want to close your account")).not.toBeInTheDocument();
  ...
  it("must call the api to close the account only on confirm button", async () => {
    ...
    server.pretender.handledRequest = (verb, path) => {
      ...
      expect(path).toBe(`${process.env.REACT_APP_BASE_API}/users/deleteparticipantaccount`);
    };
  });
  ...
  it("must redirect to the confirmation page following a successful delete", async () => {
    ...
    const confirmButtons = await screen.findAllByRole("button");
    userEvent.click(confirmButtons[0]);
    await waitForElementToBeRemoved(await screen.findByText(/Closing your account.../));
...
describe("Account closed analytics", () => {
  it("must send the correct data", async () => {
    ...
    await waitFor(
      () => {
        expect(ReactGA.testModeAPI.calls).toEqual([["send", { hitType: "pageview", page: "/MyAccount/CloseAccount" }]]);
      },
      { timeout: 1000 }
    );
  });
