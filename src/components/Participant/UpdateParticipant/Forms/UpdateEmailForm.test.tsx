...
import { render, screen, waitForElementToBeRemoved, userEvent, waitFor } from "../../../../Helper/test-utils";
...
describe("Update Email display tests", () => {
  ...
  it("must display the data retrieved from the server correctly", async () => {
    ...
    const confNewEmailEditor = await screen.findByLabelText("Confirm your new email address");
...
describe.each([
  ...
  (emailAddress, entryValidationErrorMessage, confirmationValidationErrorMessage) => {
    test(`validates ${emailAddress} correctly`, async () => {
      ...
      await waitFor(async () => {
        ...
        expect(errorMessages[1]).toHaveTextContent(confirmationValidationErrorMessage);
      });
    });
  }
);
...
describe.each([
  ...
  (emailAddress, validationErrorMessage) => {
    test(`validates ${emailAddress} correctly`, async () => {
      ...
      userEvent.click(editorButtons[0]);
      await waitFor(async () => {
        const validationErrors = await screen.findAllByText(validationErrorMessage);
...
describe("Update Email must show correct error messages", () => {
  it("must show the correct message when a user does not enter their new email address", async () => {
    ...
    userEvent.click(editorButtons[0]);
    expect(await screen.findByText("Enter your new email address")).toBeInTheDocument();
  });
  ...
  it("must show the correct message when a user does not enter a valid new email address", async () => {
    ...
    userEvent.click(editorButtons[0]);
    expect(
      await screen.findByText("Enter an email address in the correct format, like name@example.com")
    ).toBeInTheDocument();
  });
  ...
  it("must show the correct message when a user does not enter matching email addresses", async () => {
    ...
    userEvent.click(editorButtons[0]);
    expect(await screen.findByText("Enter the same email address as above")).toBeInTheDocument();
  });
